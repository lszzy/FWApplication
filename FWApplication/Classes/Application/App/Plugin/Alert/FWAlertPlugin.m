...
{
    if (!confirm) {
        confirm = FWAlertPluginImpl.sharedInstance.defaultConfirmButton ? FWAlertPluginImpl.sharedInstance.defaultConfirmButton() : [FWApplicationBundle localizedString:@"确定"];
    }
                    ...
                    priority:(FWAlertPriority)priority
{
    ...
    if (!cancel) {
        if (style == UIAlertControllerStyleActionSheet || actions.count > 0) {
            cancel = FWAlertPluginImpl.sharedInstance.defaultCancelButton ? FWAlertPluginImpl.sharedInstance.defaultCancelButton() : [FWApplicationBundle localizedString:@"取消"];
        } else {
            cancel = FWAlertPluginImpl.sharedInstance.defaultCloseButton ? FWAlertPluginImpl.sharedInstance.defaultCloseButton() : [FWApplicationBundle localizedString:@"关闭"];
        }
    }
