...
extension Wrapper where Base: UINavigationController {
    ...
    public var containerBackgroundColor: UIColor {
        get { return base.__fw.containerBackgroundColor }
        set { base.__fw.containerBackgroundColor = newValue }
    }
    
    /// 全局启用NavigationBar转场。启用后各个ViewController管理自己的导航栏样式，在viewDidLoad或viewViewAppear中设置即可
    public static func enableBarTransition() {
        Base.__fw.enableBarTransition()
    }
    ...
    public var fullscreenPopGestureEnabled: Bool {
        get { return base.__fw.fullscreenPopGestureEnabled }
        set { base.__fw.fullscreenPopGestureEnabled = newValue }
    }

    /// 导航栏全屏返回手势对象
    public var fullscreenPopGestureRecognizer: UIPanGestureRecognizer {
        return base.__fw.fullscreenPopGestureRecognizer
    }
    
    /// 判断手势是否是全局返回手势对象
    public static func isFullscreenPopGestureRecognizer(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        return Base.__fw.isFullscreenPop(gestureRecognizer)
    }
...
extension Wrapper where Base: UIViewController {
    ...
    public var barTransitionIdentifier: Any? {
        get { return base.__fw.barTransitionIdentifier }
        set { base.__fw.barTransitionIdentifier = newValue }
    }
    
    /// 视图控制器是否禁用全屏返回手势，默认NO
    public var fullscreenPopGestureDisabled: Bool {
        get { return base.__fw.fullscreenPopGestureDisabled }
        set { base.__fw.fullscreenPopGestureDisabled = newValue }
    }

    /// 视图控制器全屏手势距离左侧最大距离，默认0，无限制
    public var fullscreenPopGestureDistance: CGFloat {
        get { return base.__fw.fullscreenPopGestureDistance }
        set { base.__fw.fullscreenPopGestureDistance = newValue }
    }
