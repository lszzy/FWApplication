...
@implementation FWColorWrapper (FWApplication)
...
- (UIColor *)addColor:(UIColor *)color blendMode:(CGBlendMode)blendMode
{
    ...
    CGContextSetFillColorWithColor(context, self.base.CGColor);
...
- (BOOL)isDarkColor
{
    ...
    if (![self.base getRed:&red green:&green blue:&blue alpha:&alpha]) {
        if ([self.base getWhite:&red alpha:&alpha]) { green = red; blue = red; }
    }
...
@implementation FWColorClassWrapper (FWApplication)
...
- (UIColor *)colorWithImage:(UIImage *)image point:(CGPoint)point
{
    // 检查点是否越界
                           ...
                           direction:(UISwipeGestureRecognizerDirection)direction
{
    ...
    return [self gradientColorWithSize:size colors:colors locations:locations startPoint:startPoint endPoint:endPoint];
}
